"""Overfit ATM on 100 trials from one subject to confirm the model can learn.

If train accuracy fails to exceed 80% by epoch 50 the model has a bug and a
full training run would waste GPU time. This is a hard assertion on exit.

Usage
-----
    python sanity_check_atm.py                         # sub-02 (default)
    python sanity_check_atm.py --subject sub-05
    python sanity_check_atm.py --n-samples 100 --epochs 50 --lr 1e-3
"""
...
def main():
    ...
    ap.add_argument("--epochs", type=int, default=150,
                    help="Memorizing 100 samples on a 6.5M-param model takes "
    ...
    # Disable all dropout for the overfitting test -- we want the model to
    # memorize 100 samples, which dropout actively fights. This is only for
    # the sanity check; full training uses the paper's dropout values.
    cfg = ATMConfig(
