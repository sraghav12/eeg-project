"""Train the adapted ATM classifier on our multi-subject EEG dataset.

Hyperparameters follow the user's Phase-2 brief (hybrid recipe):
    optimizer      : AdamW, lr=1e-4, weight_decay=0.01
    schedule       : linear warmup (500 steps) then cosine decay
    grad clip      : 1.0
    batch size     : 64      (paper default; brief allowed override)
    epochs         : 40      (paper default; brief allowed override)
    normalization  : per-subject per-channel z-score at dataset level

Logs to wandb if WANDB_API_KEY is set, else stdout only.

Checkpointing: best-by-val-accuracy is saved to
    ./checkpoints/atm_sharmar_<YYYYMMDD-HHMM>.pt
...
def main():
    ...
    ap.add_argument("--dropout-proj", type=float, default=0.5,
                    help="Dropout inside the proj_eeg residual (ATMConfig.dropout_proj).")
    ...
    ap.add_argument(
        "--ckpt-dir",
        type=str,
        default="/ocean/projects/cis260086p/sharmar/atm/checkpoints",
    )
    ap.add_argument("--wandb-project", type=str, default="eeg-atm")
    ...
    run_name = args.run_name or f"atm_sharmar_{stamp}"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (eeg, y, sid) in enumerate(train_loader):
            eeg, y, sid = eeg.to(device, non_blocking=True), y.to(device), sid.to(device)
